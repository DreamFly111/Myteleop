 ...
 */
public class onOffNodeMain extends RelativeLayout implements Animation.AnimationListener, MessageListener<Odometry>, NodeMain {
    ...
    private float deadZoneRatio = 0.0F / 0;
    private float joystickRadius = 0.0F / 0;
    private float parentSize = 0.0F /0;
    ...
    private float turnInPlaceStartTheta = 0.0F / 0;
    ...
    public onOffNodeMain(Context context) {
        super(context);
        this.initOnOffNode(context);
        //this.topicName = "~cmd_vel";
        this.topicName = "on_off";
    }

    public onOffNodeMain(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.initOnOffNode(context);
//        this.topicName = "~cmd_vel";
        this.topicName = "on_off";
    ...
    public onOffNodeMain(Context context, AttributeSet attrs, int defStyle) {
...
//        this.topicName = "~cmd_vel";
    ...
    public void onNewMessage(Odometry message) {
        ...
        if(this.turnInPlaceMode) {
            this.post(new Runnable() {
                public void run() {
                    onOffNodeMain.this.updateTurnInPlaceRotation();
                }
            });
            this.postInvalidate();
        }
    ...
    private void initOnOffNode(Context context) {
       /* this.setGravity(17);
...
*/
    ...
    private void updateTurnInPlaceMode() {
       /* if(!this.turnInPlaceMode) {
            ...
            (new Timer()).schedule(new TimerTask() {
                public void run() {
                    onOffNodeMain.this.post(new Runnable() {
                        public void run() {
                            if(onOffNodeMain.this.turnInPlaceMode) {
                                onOffNodeMain.this.currentRotationRange.setAlpha(1.0F);
                                onOffNodeMain.this.previousRotationRange.setAlpha(1.0F);
                                onOffNodeMain.this.intensity.setAlpha(0.2F);
                            }

                        }
                    });
                    onOffNodeMain.this.postInvalidate();
                }
            }, 200L);
    ...
    public void onStart(ConnectedNode connectedNode) {
        ...
        this.publisherTimer.schedule(new TimerTask() {
            public void run() {
                if(onOffNodeMain.this.publishVelocity) {
                    onOffNodeMain.this.publisher.publish(onOffNodeMain.this.currentVelocityCommand);
                }
